...
@section('content')
    @include('partials.page-hero', [
        'eyebrow' => __('site.nav.news'),
        'title' => __('site.news.title'),
        'subtitle' => __('site.news.subtitle'),
        'icon' => 'newspaper',
    ])
    ...
    <section class="mx-auto max-w-7xl px-4 py-14 sm:py-16">
        <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($articles as $item)
                ...
                @endphp
                <a href="/{{ $locale }}/news/{{ $item->slug }}" class="reveal-item group flex flex-col overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-1.5 hover:shadow-xl hover:ring-brand-200">
